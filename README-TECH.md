...
|-----------|-------------|
| [README.md](./README.md) | Vista general para visitantes: qué es D-Lab y qué publica, sin tecnicismos |
...
| [README-TECH.md#fase-101--exposición-pública-de-servicios](./README-TECH.md#fase-101--exposición-pública-de-servicios) | Fase 10.1: Exposición pública de servicios K8s vía nginx DV0 + Let's Encrypt |
...
- Grafana (estado actual, `verificado 2026-08-18`): usuario `elarreglador` (rol Admin + Grafana Admin global) garantizado por `scripts/grafana-user.sh`, con las passwords en `info_sensible/grafana-user.env` (gitignored); `admin` de mantenimiento cuya password vive en el Secret `kube-prometheus-stack-grafana` (`monitoring`). El fichero `values-monitoring.yaml` (con `adminPassword`) **ya no existe** en los hosts k8s-master-1/-2.
...
- **Sección «Servicios D-Lab»** (2026-08-20): tarjetas de los servicios del cluster (Multimedia, Grafana, Node-RED, MariaDB, Radio SDR) más la tarjeta **«IA local (Ollama)»** (`dev.svg`): modelos de lenguaje autoalojados, de uso interno (sin acceso desde el exterior) — coherente con la exposición real de Ollama (solo LAN/WG, sin ingress, ver [03-Aplicaciones.md](./03-Aplicaciones.md)).
- **Sección «Monitorización en vivo»** (2026-08-18, **ampliada 2026-08-31** a 11 paneles): embebe en un `<iframe>` el **public dashboard** de Grafana «Sistema D-Lab» — 7 paneles CPU/RAM (DV0 w24 + 2 filas 3× w8) + **4 de almacenamiento 24h** en matriz `2×2 w12` (izquierda `bargauge` PVC 400Gi `h:3 w:12` `75/90` sobre `h:7 w:12` tráfico PVC agregado `h:7`, derecha `Tráfico sda worker-1 h:5 w:12` sobre `worker-2 h:5 w:12`) con `src="https://grafana.elarreglador.eu/public-dashboards/<accessToken>?theme=dark&from=now-24h&to=now"` (tema fijo oscuro, ventana de 24 h). Altura del iframe `1050px` (`files/landing/styles.css:496`, `aspect-ratio 24/14`, `cache-busting v=20260831g` verificado `curl 200` `public-dashboards 200`). El `index.html` lleva el placeholder `__PUBLIC_DASHBOARD_URL__`; `deploy-landing.sh` lo sustituye en **staging** (`mktemp`) por la URL real leída de `info_sensible/public-dashboard.env` (gitignored). Requisitos Grafana: `allow_embedding = true` + feature toggle `publicDashboards`; registro efímero garantizado por `scripts/ensure-public-dashboard.sh`. **Ojo Grafana 13**: ruta `/public-dashboards/<token>` (guion). **Gotcha**: `timeSelectionEnabled=false` ignora `from/to` de URL, ventana real es `"time"` en `files/monitoring/grafana-dashboard-sistema-dlab.yaml` (`now-24h`); para cambiarla editar `sistema-dlab.json time` y reaplicar ConfigMap (sidecar kiwigrid `Writing /tmp/dashboards/sistema-dlab.json reload 200`). PromQL almacenamiento: gauge `100*(1 - node_filesystem_avail_bytes{mountpoint="/mnt/data"} / node_filesystem_size_bytes{… instance="192.168.1.31:9100"})` y tráfico `sum(rate(node_disk_read|written_bytes_total{device="sda",instance=~"192.168.1.3[12]:9100"}[5m]))` agregado PVC + `rate(...instance="192.168.1.3[12]:9100")` per-worker (`binBps` → MiB/s) — ver [03-Aplicaciones.md](./03-Aplicaciones.md) § Grafana.
...
**Nota sobre roles (verificado 2026-08-20, actualizado 2026-08-29)**: ambos control-planes (`k8s-master-1` y `k8s-master-2`) tienen la etiqueta `node-role.kubernetes.io/control-plane` y el taint `NoSchedule`; el scheduler no ubica pods de usuario en los masters. Hasta el **2026-08-20** `k8s-master-1` carecía de label/taint y alojaba pods de usuario (qBittorrent, landing, es-badge); se blindó el 2026-08-20. Movilidad de pods: **qBittorrent, Jellyfin y aMule** corren en cualquier worker (`eu.elarreglador/worker=true`, config en NFS `nfs-storage-v4`); el stack multimedia es 100 % móvil (simplificado 2026-08-29, sin cadena *arr; ver `03-Aplicaciones.md` y `files/multimedia/_retirado/`). **rtl-sdr** anclado por hardware (`eu.elarreglador/sdr=true`). Historial del anclaje *arr/Jellyseerr (SQLite sobre Gluster-HDD no viable) en git log y `_retirado/`.
...
- [x] **Fase 10.1: Exposición Pública de Servicios**:
  ...
  - Detalle en [README-TECH.md#fase-101--exposición-pública-de-servicios](./README-TECH.md#fase-101--exposición-pública-de-servicios)
...
- [x] **Fase 12: Monitoreo y Observabilidad** (ampliada 2026-08-31 a 11 paneles):
  ...
  - Detalle en [README-TECH.md#fase-12--monitoreo-y-observabilidad](./README-TECH.md#fase-12--monitoreo-y-observabilidad) y [03-Aplicaciones.md](./03-Aplicaciones.md) § Grafana
