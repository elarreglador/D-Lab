...
- **Límites P2P para priorizar Jellyfin** (`verificado 2026-09-01`, fibra DIGI medida ~600 Mbit/s simétricos `01-Network.md#ancho-de-banda-y-límites-p2p-para-priorizar-multimedia`): **1/3 del ancho para P2P (200 Mbit/s), 2/3 libres (400 Mbit/s) para `jellyfin 192.168.1.53:8096`**. qBittorrent `19500 KiB/s down/up` (`19968000 B/s`, `Session\GlobalDLSpeedLimit=19500` modo 0, antes `5 KiB/s` modo 1 cerrado) y aMule `MaxDownload=4880 / MaxUpload=4880` (40 Mbit/s, antes `0` ilimitado). Total P2P 200 Mbit/s = 33% del total. Verificado `curl qbittorrent:8080/api/v2/transfer/info → 19968000` y `amule.conf → 4880` tras `rollout restart`.
...
- **Dashboard provisionado «Sistema D-Lab»** (`files/monitoring/grafana-dashboard-sistema-dlab.yaml`, ConfigMap `grafana-dashboard-sistema-dlab` en `monitoring` con label `grafana_dashboard: "1"`): **11 paneles (verificado 2026-08-31) — 7 tarjetas stat de CPU%/RAM% + 4 de almacenamiento**, ventana fija `now-24h` (`refresh 1m`, `uid sistema-dlab`, `editable false`). **CPU%/RAM% (7, `thresholds 60/85`):** DV0 `w24 h5 y:0` (job `dv0-host` `host="dv0"`) + filas `y:5` (D1 Host `job host-node host=d1` + `k8s-master-1 192.168.1.21:9100` + `k8s-worker-1 192.168.1.31:9100`) y `y:10` (D2 Host `host-node host=d2` + `k8s-master-2 192.168.1.22:9100` + `k8s-worker-2 192.168.1.32:9100`), 3× `w8 h5` por fila. PromQL `100 - avg by (host|instance) (rate(node_cpu_seconds_total{mode="idle"}[5m]))*100` y `100*(1 - node_memory_MemAvailable_bytes/node_memory_MemTotal_bytes)`. **Almacenamiento (4, layout 2×2 `w12` `y:15→25`):** `id 8` bargauge `media-data 400Gi` `h:3 w:12 x:0 y:15` `unit percent` `thresholds 75 amarillo / 90 rojo` `displayMode gradient horizontal`; `id 10` `Tráfico PVC 400Gi` `h:7 w:12 x:0 y:18` `unit binBps` `palette-classic`; `id 9` `Tráfico sda worker-1` `h:5 w:12 x:12 y:15` e `id 11` `Tráfico sda worker-2` `h:5 w:12 x:12 y:20` (`rate(node_disk_read|written_bytes_total{device="sda",job="node-exporter",instance="192.168.1.3[12]:9100"}[5m])` per-worker, `sum(rate(...))` para PVC agregado `Lectura/Escritura PVC 400Gi`). **Gotcha `kubelet_volume_stats_*` (verificado 2026-08-31):** `kubelet_volume_stats_used_bytes{namespace="multimedia",persistentvolumeclaim="media-data"}` y `capacity` devuelven `vector []` para NFS (`nfs-subdir-external-provisioner` no expone volume-stats); se usa aproximación fiel del brick `100*(1 - node_filesystem_avail_bytes{mountpoint="/mnt/data",fstype="ext4",job="node-exporter",instance="192.168.1.31:9100"} / node_filesystem_size_bytes{…})` (sda1, único uso del disco). El sidecar kiwigrid re-provisiona al aplicar el CM; `kubectl logs deploy/kube-prometheus-stack-grafana -c grafana-sc-dashboard` → `Writing /tmp/dashboards/sistema-dlab.json` `reload 200`.
...
- **Public dashboard de la landing** (`verificado 2026-08-18`, **ampliado 2026-08-31** a 11 paneles): el dashboard «Sistema D-Lab» se expone sin login para incrustarlo en la landing (sección *Monitorización en vivo*, ahora con 4 paneles de almacenamiento: gauge 400Gi `75/90` + tráfico PVC agregado `h7` + sda worker-1/2 `h5` `w12` 24h). En el grafana.ini se añadieron `security.allow_embedding = true` (sin `X-Frame-Options`) y el feature toggle `publicDashboards`. El registro del public dashboard es **efímero** (se pierde al recrear el pod): lo garantiza `scripts/ensure-public-dashboard.sh` (idempotente; guarda la URL en `info_sensible/public-dashboard.env`, gitignored). **Ojo Grafana 13**: la ruta pública usa **guion** — `https://grafana.elarreglador.eu/public-dashboards/<token>`; la variante con barra (`/public/dashboards/`) cae en el handler de assets y responde 302 a `/login`. **Gotcha ventana temporal**: el public dashboard se creó con `timeSelectionEnabled=false`, así que **ignora los `from`/`to` de la URL del iframe** y muestra el rango guardado del dashboard (`time from now-24h` en `sistema-dlab.json`, verificado 2026-08-31 `max y+h 25`). Para cambiar la ventana se edita `"time"` en `files/monitoring/grafana-dashboard-sistema-dlab.yaml` y se reaplica el ConfigMap (el sidecar re-provisiona; el registro público se conserva). La landing embebe con `__PUBLIC_DASHBOARD_URL__?theme=dark&from=now-24h&to=now` resuelto por `scripts/deploy-landing.sh` y muestra `height 1050px` (`files/landing/styles.css:496`, `v=20260831g`, `aspect-ratio 24/14`) — verificado `curl 200` y `public-dashboards/<token> 200`.
...
- **Datos de los paneles**: Grafana **consulta** a Prometheus y AlertManager (no al revés: Prometheus recolecta y almacena; Grafana hace las consultas cuando pinta un panel). Datasources definidos en el ConfigMap `kube-prometheus-stack-grafana-datasource` (montado por el sidecar kiwigrid):
  ...
  - Ambas son ClusterIP internas (`:9090` y `:9093`), comunicación pod↔pod por la red del cluster; **no** necesitan dominio público. **Paneles de almacenamiento (verificado 2026-08-31):** gauge PVC `100*(1 - node_filesystem_avail_bytes{mountpoint="/mnt/data",fstype="ext4",job="node-exporter",instance="192.168.1.31:9100"} / node_filesystem_size_bytes{…})` (`percent`, `75/90`); tráfico PVC agregado `sum(rate(node_disk_read|written_bytes_total{device="sda",instance=~"192.168.1.3[12]:9100"}[5m]))` (`binBps` → MiB/s) y per-worker `rate(node_disk_read|written_bytes_total{device="sda",instance="192.168.1.31:9100"}[5m])` / `.32:9100`.
...
- **Nota almacenamiento (verificado 2026-08-31):** el dashboard de almacenamiento no usa `kubelet_volume_stats_*` (vacío para NFS, ver Grafana), sino `node-exporter` `node_filesystem_avail/size_bytes{mountpoint="/mnt/data"}` y `node_disk_read|written_bytes_total{device="sda"}` (ver sección Grafana para PromQL exactas).
...
- **node-exporter de hosts (D1/D2)**: Service headless `host-node` (9100/19100) + Endpoints manuales + ServiceMonitor `host-node`. D2 se scrapea directo (`:9100`); D1 vía relay socat en D2 (`TCP4-LISTEN:19100 → TCP4:192.168.1.11:9100`, servicio systemd `node-exporter-relay-d1`). Los containers LXD usan macvlan y no alcanzan a su propio host. Provee además `node_filesystem_*` (`/mnt/data`) y `node_disk_*` (`sda`) para el dashboard de almacenamiento (ver Grafana).
...
- **node-exporter de workers (sda)**: DaemonSet `kube-prometheus-stack-prometheus-node-exporter` en `k8s-worker-1/2` (`192.168.1.31:9100` / `.32:9100`) expone `node_filesystem_size/avail_bytes{mountpoint="/mnt/data"}` (sda1, 491105460224) y `node_disk_read|written_bytes_total{device="sda"}` usados por los 4 paneles de almacenamiento (`w12 h3/h5/h7`, 24h).
