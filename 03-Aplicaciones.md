...
- **Política de idioma de descargas: castellano** (`verificado 2026-08-17`): Sonarr v4 y Radarr v6 no tienen *language profiles*, así que el idioma se decide con Custom Formats. `scripts/multimedia-language.sh` (idempotente y declarativo: crea o actualiza los CFs y puntúa el perfil) configura en ambas apps los CFs **"Español (Audio)"** (Language=Spanish/castellano, score 110), **"Audio dual"** (regex `dual|multi|es-en|castellano…inglés`, score 110) y **"VOSE"** (regex de subtítulos, score 100) y aplica `minFormatScore=100` al perfil **"Any"** (id 1, el único que usa Jellyseerr vía `activeProfileId=1`). Jerarquía resultante (score por release, se elige el mayor): doblado+dual+subs **320** > doblado+dual **220** > doblado+subs **210** > doblado **110** > VOSE **100** > inglés sin subs **0** (rechazado). El CF "VOSE" **no** exige idioma a propósito: los releases `...-VOSE`/`SUBBED`/`VOS` se parsean a menudo como idioma "Unknown" (no inglés) y exigirlo los dejaría sin puntuar — validado con `/api/v3/parse` (ej. `...-VOSE` → CF VOSE; `...-SPANISH` → Español; `...-ENGLISH` → 0). Limitaciones: la detección es por nombre (un release sin etiqueta DUAL/SUBS no puntúa; `SubSpanish` se cuenta como doblaje por parsear `Spanish`); "Spanish (Latino)" queda excluido del CF de doblaje; Radarr no expone CFs en `/api/v3/parse` (mismo motor de matching, verificado por configuración almacenada).
