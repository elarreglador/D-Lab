...
- `deploy-landing.sh`: `files/landing/` → ConfigMap `landing-html`. Claves planas en `binaryData` base64 (las fuentes `.woff2` en raíz: ConfigMap no admite `/`). Guard por bandas frente al techo de etcd (~1,5 MiB/objeto): avisa desde ~0,9 MiB y aborta cerca de ~1,35 MiB (no bloquea en 1 MiB). Aplica con `kubectl apply --server-side` (el client-side duplicaría el contenido en `last-applied-configuration`, tope 256 KiB, y rechazaría el apply).
